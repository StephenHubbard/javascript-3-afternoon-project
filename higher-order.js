...
//Code Here
let myStrongest = monstersInYourPocket.filter((el, i, arr) => arr[i].CP >= 200 ? true : false)
...
let orderTotals = orders.map((el, i, arr) => arr[i].price * (1 + arr[i].tax))
...
let bobsTotal = purchases.reduce((acc, el) => {
  if (el.owner === 'Bob') {
    acc += el.price;
  }
  return acc
